...
function HomePage() {
  ...
  // Define COLORS array within the component
  ...
  const { monthlyData, expenseCategories, investments, totalIncomeOverview, totalExpensesOverview, netSavingsOverview, filteredTransactions, monthlySummary, allMonths } = useMemo(() => {
    // Initialize default values
    let monthlyData = [];
    let expenseCategories = [];
    let investments = []; // Placeholder for investments, as backend doesn't provide it yet

    // Process transactions to generate monthly data for the chart
    const monthlyAggregates = transactions.reduce((acc, t) => {
      const date = new Date(t.date);
      const monthNameShort = date.toLocaleString('default', { month: 'short' });

      if (!acc[monthNameShort]) {
        acc[monthNameShort] = { name: monthNameShort, income: 0, expenses: 0 };
      }
      if (t.type === 'income') {
        acc[monthNameShort].income += parseFloat(t.amount);
      } else {
        acc[monthNameShort].expenses += parseFloat(t.amount);
      }
      return acc;
    }, {});

    // Ensure all 12 months are present in monthlyData, even if no transactions
    const fullYearMonthsShort = Array.from({ length: 12 }).map((_, i) => new Date(0, i).toLocaleString('default', { month: 'short' }));
    monthlyData = fullYearMonthsShort.map(month => monthlyAggregates[month] || { name: month, income: 0, expenses: 0 });
    ...
    // Calculate totals for overview cards based on filterMonth
    let currentOverviewIncome = 0;
    let currentOverviewExpenses = 0;

    // Filter transactions based on the selected month for consistent calculations
    const transactionsForOverview = filterMonth === 'all'
      ? transactions
      : transactions.filter(t => new Date(t.date).toLocaleString('default', { month: 'long' }).toLowerCase() === filterMonth.toLowerCase());

    currentOverviewIncome = transactionsForOverview.filter(t => t.type === 'income').reduce((sum, t) => sum + parseFloat(t.amount), 0);
    currentOverviewExpenses = transactionsForOverview.filter(t => t.type === 'expense').reduce((sum, t) => sum + parseFloat(t.amount), 0);
    const currentOverviewNetSavings = currentOverviewIncome - currentOverviewExpenses;


    // Filter transactions for the "Recent Transactions" table and "Monthly Summary" card
    const filteredTransactions = filterMonth === 'all'
      ? transactions
      : transactions.filter(t => new Date(t.date).toLocaleString('default', { month: 'long' }).toLowerCase() === filterMonth.toLowerCase());

    // Calculate summary specifically for the "Monthly Summary" card
    const summaryForMonthlySummaryCard = filteredTransactions.reduce((acc, transaction) => {
    ...
    // Determine the month name for the "Monthly Summary" card title
    const currentMonthLongName = new Date().toLocaleString('default', { month: 'long' });
    const monthlySummary = summaryForMonthlySummaryCard[filterMonth === 'all' ? currentMonthLongName : new Date(Date.parse(filterMonth + " 1, 2000")).toLocaleString('default', { month: 'long'})] || { income: 0, expenses: 0 };


    // Generate list of all months for the filter dropdown
    ...
    return {
      ...
      expenseCategories,
      investments,
      totalIncomeOverview: currentOverviewIncome,
      totalExpensesOverview: currentOverviewExpenses,
      netSavingsOverview: currentOverviewNetSavings,
      filteredTransactions,
      monthlySummary: monthlySummary,
      allMonths
    };
  }, [transactions, filterMonth, COLORS]); // COLORS added to dependency array for completeness
    ...
    <div className="min-h-screen bg-gray-100 font-inter p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white">
          ...
          <CardContent>
            <div className="text-3xl font-bold">${(totalIncomeOverview - totalExpensesOverview).toFixed(2)}</div>
        ...
        <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
          ...
          <CardContent>
            <div className="text-3xl font-bold">${totalIncomeOverview.toFixed(2)}</div>
        ...
        <Card className="bg-gradient-to-br from-red-500 to-red-600 text-white">
          ...
          <CardContent>
            <div className="text-3xl font-bold">${totalExpensesOverview.toFixed(2)}</div>
        ...
        <Card className="bg-gradient-to-br from-purple-500 to-purple-600 text-white">
          ...
          <CardContent>
            <div className="text-3xl font-bold">${netSavingsOverview.toFixed(2)}</div>
            <p className="text-xs text-white opacity-80">{netSavingsOverview >= 0 ? 'Positive savings' : 'Negative savings'}</p>
          </CardContent>
        </Card>
      </div>
